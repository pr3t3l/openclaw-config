FINANCE TRACKER v2 — ESPECIFICACIÓN FINAL
Consolidación de análisis: Claude + GPT + Gemini + decisiones del owner

1. EL PROBLEMA CENTRAL (POR QUÉ EXISTE ESTE DOCUMENTO)
El Finance Tracker v1 funciona. Tiene 38 comandos, 87+ reglas, Google Sheets, crons, telemetría. Pero el proceso de instalación y setup depende de que el LLM interprete correctamente un SKILL.md largo y siga el flujo conversacional en orden. Esto produce tres fallos reales:
El primero es inconsistencia entre modelos. GPT-5.4 hace el setup en un orden, Sonnet 4.6 en otro, M2.7 se salta pasos. El SKILL.md es una sugerencia, no un contrato ejecutable. Cada modelo tiene su propia idea de qué significa "pregunta una cosa a la vez" y cuándo el usuario terminó de hablar.
El segundo es fragilidad ante interrupciones. Si el usuario cierra Telegram a mitad del setup y vuelve al día siguiente, el agente no sabe en qué paso estaba. Empieza de nuevo o improvisa. No hay checkpoint ni estado persistente.
El tercero es ausencia de validación. El LLM acepta cualquier cosa que el usuario diga sin verificar que sea parseable, completa o coherente. Si el usuario escribe "tengo deudas varias" en lugar de dar números, el LLM puede inventar un monto o simplemente seguir adelante.
La solución no es un mejor prompt. Es mover la lógica de orquestación a código Python determinista y dejar al LLM haciendo solo lo que hace bien: entender texto libre y convertirlo a JSON estructurado.
