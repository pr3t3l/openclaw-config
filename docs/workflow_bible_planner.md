...
### Sources: extraction + master_plan_v3 + upgrade_v3.1 + build phases 0-3 + Bible v2 §10
...
| Script | Purpose |
|--------|---------|
| `spawn_planner_agent.py` | Spawns individual agents via LiteLLM streaming curl |
| `spawn_debate.py` | Multi-model debate with judge + optional red team |
...
| `run_phase_c.sh` | Execute Fase C (Buildability): C1→C2→C3 |
...
| `build_fact_pack.py` | Build context pack for agents |
...
| `generate_implementation_by_blocks.py` | Generate implementation plan in blocks |
...
All 8 core agents use `claude-sonnet46` via LiteLLM.
...
Report generator: narrator=claude-sonnet46, reviewer=chatgpt-gpt54 `[AUDIT]`

---

# 9. TOKEN TRACKING `[EXT]`
...
Pricing dict in spawn scripts: `[EXT]`

| Model | Input $/1M | Output $/1M |
|-------|-----------|------------|
| claude-sonnet46 | $3.00 | $15.00 |
| claude-opus46 | $5.00 | $25.00 |
| gpt52-medium | $3.00 | $12.00 |
| gemini31pro-none | $1.25 | $10.00 |
| minimax-m27 | $0.30 | $1.20 |

**NOTE:** These are the planner's internal pricing estimates. With Codex OAuth, GPT models are actually $0/token. Planner pricing may overestimate GPT costs. `[AUDIT]`

---

# 10. COMPLETED RUNS `[AUDIT]`
...
├── scripts/                            — 22 scripts
│   ├── spawn_planner_agent.py
│   ├── spawn_debate.py
│   ├── cost_estimator.py
│   ├── start_plan.sh / run_phase_a/b/c.sh / run_full_plan.sh
│   ├── resume_plan.sh
│   ├── validate_schema.py / human_gate.py / generate_report.py
│   └── [repair/retry scripts]
...
### Working ✅
- 14 skills deployed `[AUDIT]`
- 22 scripts present `[AUDIT]`
- 9 JSON schemas for validation
- 4 completed runs (2 E2E test, 2 production)
- Real token tracking in manifest.json
- **3-round debate LIVE** (propose → cross-critique → consolidate) `[VERIFIED]`
- **Iterative intake LIVE** (5 rounds × 3 questions) `[VERIFIED]`
- **Human gates with adjust LIVE** `[VERIFIED]`
- **Two analysis levels LIVE** (regular vs deep) `[VERIFIED]`
- **Report generator LIVE** `[VERIFIED]`
- Own bot: @Super_Workflow_Creator_bot
- Version tracked in `scripts/VERSION`

### Issues ⚠️
- **CEO AGENTS.md says Fases B+C "not implemented"** — INCORRECT, they are live. Need to update CEO. `[AUDIT]`
- **Planner pricing overestimates GPT** — uses pre-OAuth prices, actual is $0/token via Codex `[AUDIT]`
- **spawn_core.py** in shared/ is a pinned copy — must be manually synced if upstream changes

### Not implemented
- ~~3-round debate~~ → ✅ LIVE
- ~~Iterative intake~~ → ✅ LIVE
- ~~Real human gates~~ → ✅ LIVE
- ~~Two analysis levels~~ → ✅ LIVE
...
| 🔴 | Update models.json pricing for Codex OAuth ($0 for GPT) | None | 10 min |
...
| 🟢 | Sync spawn_core.py if upstream version has changed | Check | 15 min |
